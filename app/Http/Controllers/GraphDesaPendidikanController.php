...
class GraphDesaPendidikanController extends Controller
{
    //
    ...
    public function cards($desa_id){
		...
		foreach($tipe_sekolah as $ts){
			$sekolah = DesaToSekolah::leftJoin('sekolahs', 'desa_to_sekolahs.sekolah_id', 'sekolahs.id')
				...
				->where('desa_to_sekolahs.desa_id', $desa_id)
	...
	public function jumlahsiswaperkelas($desa_id){
        $tahun_ajaran = date('Y');
		$query = "SELECT murid_sekolahs.kelas as kelas, sum(murid_sekolahs.jumlah) as sum_jumlah
            ...
            WHERE murid_sekolahs.tahun_ajaran = $tahun_ajaran
            AND desa_to_sekolahs.desa_id = $desa_id
	...
	public function guruvsmurid($desa_id){
		$tipe_sekolah = ['tk', 'sd', 'smp', 'sma'];
		$murids = [];
		$gurus = [];
		$perbandingans = [];
	...
	public function negerivsswasta($desa_id){
	...
	public function listsekolah($desa_id){
